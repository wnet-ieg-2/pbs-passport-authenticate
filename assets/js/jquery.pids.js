      ...
      if (user.thumbnail_URL) {
        ...
        if ($(".pbs_passport_authenticate div.messages-new")[0]){
            ...
            if (user.thumbnail_URL) {thumbimage = "<span><img src=" + user.thumbnail_URL + " /></span>";}	
             welcomestring = '<a href="' + userinfolink + '" class="userlink">' + thumbimage + '<span class="name"><span class="nm">' + user.first_name + '</span></span></a> <ul class="user-drop"><li><a href="' + userinfolink + '">USER STATUS</a></li><li><a  class="signout">SIGN OUT</a></li></ul>';
        ...
        $(".passportcoveplayer").each(function (i) {
          if (typeof($(this).data('window')) !== 'undefined') {
            ...
            if (videoWindow != 'public' && videoWindow != '' && !$(this).hasClass("playing")) {
              $(this).html('<div class="embed-container video-wrap"><iframe id="partnerPlayer_'+ i +'" frameborder="0" marginwidth="0" marginheight="0" scrolling="no" allowfullscreen="allowfullscreen" src="//player.pbs.org/widget/partnerplayer/'+videoID+'/?chapterbar=false&endscreen=false&uid='+user.pid+'&callsign='+station_call_letters_lc+'"></iframe></div>');
