...
function build_passport_player($video) {
  ...
  if (empty($video->tp_media_object_id)) {
    ...
    return '<div class="passportcoveplayer" data-window="'.$video->window.'" data-media="'.$video->tp_media_object_id.'" data-callsign="' . $call_letters . '"><div class="passport-'.$passportGated.'-video"><picture><source srcset="' . strtok($large_thumb, '?') . '?crop=1600x900&amp;format=webp" type="image/webp"><source srcset="' . strtok($large_thumb, '?') . '?crop=1200x675&amp;format=jpg" type="image/jpg"><img src="' . $large_thumb . '" width="1200" height="675" loading="eager" fetchpriority="high" alt="" style="aspect-ratio: 16/9;"></picture>' . $passportOverlay . $passportError . '</div></div>';
   }
}
